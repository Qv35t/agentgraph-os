...
const russianLabels: Record<string, string> = {
  ...
  "CLIENT PREFERENCES": "НАСТРОЙКИ КЛИЕНТА", "Settings": "Настройки", "LOCAL MULTIMODAL": "ЛОКАЛЬНАЯ МУЛЬТИМОДАЛЬНОСТЬ", "Vision workspace": "Рабочее пространство Vision",
};
...
function GraphEditor({ value, onChange }: { value: GraphDefinition; onChange: (graph: GraphDefinition) => void }) {
  ...
  function addNode(type = "agent") { setNodes((current) => { const number = current.length + 1; const next = [...current, { id: `${type}-${number}`, type, position: { x: 100 + current.length * 110, y: 150 }, data: { label: type === "vision-analyze" ? "Vision Analyze" : `Agent ${number}` } }]; sync(next, edges); return next; }); }
  return <div className="graph-editor"><div className="graph-label">Workflow graph <span>Drag nodes or connect handles to save layout semantics.</span><button type="button" className="add-node" onClick={() => addNode()}><Plus size={13} /> Agent</button><button type="button" className="add-node" onClick={() => addNode("vision-analyze")}><Plus size={13} /> Vision</button></div><ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodeChanges} onEdgesChange={onEdgeChanges} onConnect={onConnect} fitView><Background gap={18} size={1} /><Controls showInteractive={false} /></ReactFlow></div>;
}
